...
PYBIND11_MODULE(nwhy, m) {
    ...
    m.def("convert_to_s_overlap", &convert_to_s_overlap, "A function which converts a hypergraph to its s line graph",
    py::arg("x"), py::arg("y"), py::arg("data") = std::vector<int>(0), py::arg("s") = 1);
}