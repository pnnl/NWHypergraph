...
int main(int argc, char* argv[]) {
  ...
  for (auto&& file : files) {
    ...
    size_t s = 1, int num_bins = 32) {
      ...
      case 0:
      {
          nw::graph::edge_list<undirected> &&linegraph = to_two_graph_efficient_parallel_clean<undirected>(std::execution::par_unseq, hyperedges, hypernodes, hyperedge_degrees, s, num_bins);
